...
def extract_raw_code(content):
    # find method-level code containing the API call to either SecretKeyFactory, MessageDigest, or Cipher
    pattern = r'((public|private|protected).*\s*(MessageDigest|SecretKeyFactory|Cipher|Mac)\s*.*\(.*\).*})'
