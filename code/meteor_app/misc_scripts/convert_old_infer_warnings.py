...
# START and END
try:
    target_indexes = [int(item) for item in os.environ['TARGET'].split(',')]
except:
    target_indexes = [0, 3, 4, 5, 6, 7, 8, 11, 12, 14, 15, 21, 25, 27, 30, 40, 43, 44, 51, 52, 53, 55, 57, 73, 87, 106, 116, 164, 167, 170, 178, 181, 199, 209, ]   
...
def convert_to_surf_json(github_repo, commit, infer_warnings):
    ...
    num_tests = 0
    count = 0
    for warning_i, infer_warning in enumerate(infer_warnings):
        ...
        if warning_i not in target_indexes:
            continue
            ...
            #     found_source = False
                ...
                if start_recording:
                    ...
                    if not trace_content:
                        ...
                        if 'start of procedure ' in trace['description'] or 'Definition of ' in trace['description']:
                            if 'start of procedure ' in trace['description']:
                                trace_expanded['methodName'] = trace['description'].split('start of procedure ')[1].split('(')[0]
                            else:
                                trace_expanded['methodName'] = trace['description'].split('Definition of ')[1].split('(')[0]
        ...
        json_obj = {
            ...
            'qualifier': 'Resource of type $$' + source + '/$ acquired by call to @@' + sink + '/@ is not released',
            # ('$$' +source + '/$ could ' + null_verb + ' null, and is @@dereferenced by ' +sink + '/@') if source != sink else ('$$' + source + '/$ could ' + null_verb + ' null, and is @@dereferenced/@'),
            'steps': [
                {
...
def insert_code_for_combined_records(github_repo, commit, combined_records, record):
    ...
    for line_i in range(min_line_number - 2, max_line_number + 1):
        if (line_i) in combined_records[-1]['line_numbers'] and split_content[line_i].strip() != '}':
            combined_records[-1]['snippet'] += str(line_i ) + '.' +  '> ' + split_content[line_i] + '\n'
        else:
            combined_records[-1]['snippet'] += str(line_i ) + '.' +  split_content[line_i] + '\n'
...
def load_infer_warnings(infer_warning_path, warning_type):
    ...
    infer_warnings = [infer_warning for infer_warning in infer_warnings if infer_warning['bug_type'] == warning_type]
